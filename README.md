...
- **Suggest a pattern without authoring it yet.** Append an entry to [`pattern-todo.json`](pattern-todo.json) (validated against [`pattern-todo.schema.json`](pattern-todo.schema.json)) naming the candidate, the composition that raised it, and the upstream evidence. Candidates graduate from `proposed` → `drafting` → `authored` as they are written into `patterns-src/`. Rejected candidates are moved to [`pattern-todo-archive/`](pattern-todo-archive/) as one JSON file per id, keeping the rejection rationale on record without crowding the active todo.
