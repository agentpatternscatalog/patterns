...
- **[Qodo Cover (qodo-cover)](https://github.com/qodo-ai/qodo-cover)** _available_ — Open-source TestGen-LLM-inspired agent that augments an existing suite with edge-case tests, targeting the premise-violating inputs a happy-path suite never covers; notably it only keeps tests that pass current code, so it can also reinforce a hidden assumption.
- **[Diffblue Cover](https://www.diffblue.com/home/)** _available_ — Autonomous Java unit-test agent that writes characterization tests asserting the observable behaviour of code, pinning what generated code actually does so a silent premise becomes a named, regression-guarded assertion.
- **[CodeRabbit](https://www.coderabbit.ai/)** _available_ — AI PR-review agent that flags edge cases, off-by-ones, and validation gaps in a diff before merge, naming on the page the assumptions a fluent generated change would otherwise pass review without surfacing.
- **[CrossHair](https://github.com/pschanely/CrossHair)** _available_ — Symbolic-execution property checker for Python that drives functions toward inputs violating their stated contracts and assertions, exposing the empty-collection / unsorted-input premises a model leaves silent.
...
- _complements_ **Behavior-Pinning Test Before Agent Edit** — That anti-pattern is the disease — a hidden premise ships unguarded; pinning the existing behaviour as exact golden tests before the edit is the cure side, catching the silent mutation at the gate.
...
- [Is Your Code Generated by ChatGPT Really Correct? Rigorous Evaluation of Large Language Models for Code Generation](https://arxiv.org/abs/2305.01210) — Liu, Xia, Wang, Zhang, 2023
- [Do Users Write More Insecure Code with AI Assistants?](https://arxiv.org/abs/2211.03622) — Perry, Srivastava, Kumar, Boneh, 2022
